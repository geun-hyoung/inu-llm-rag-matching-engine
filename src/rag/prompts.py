...
1. target (연구 대상)
   - 정의: 이 연구가 다루는 핵심 물체, 시스템, 또는 현상
   - 핵심 질문: "이 연구는 무엇을 대상으로 하는가?"
   - 판단 힌트: 연구의 주어 역할을 하는 실체

   - 포함:
     • 물리적 장치/소재: 센서, 배터리, 양극재, 로봇
     • 시스템: 배터리 관리 시스템, 추천 시스템
     • 소프트웨어/모델: 감성분석 모델, 분류기
     • 데이터: CT 영상, 유전체 데이터

   - 제외:
     • 행위/과정: 분석, 처리, 측정, 학습 (→ method)
     • 목표/효과: 정확도 향상, 수명 연장 (→ objective)

   - 경계 케이스:
     ✓ "배터리 관리 시스템" (시스템 = 실체)
     ✗ "배터리 관리 방법" (방법 = 행위 → method)
     ✓ "토크 센서" (장치 = 실체)
     ✗ "토크 측정" (측정 = 행위 → method)

2. problem (문제)
   - 정의: 연구 대상(target)에서 발생하는 기존의 한계나 부정적 현상
   - 핵심 질문: "연구 대상의 어떤 문제를 해결하려 하는가?"
   - 판단 힌트: 대조 표현("기존", "however", "but")과 함께 나오는 부정적 서술

   - 포함:
     • 명시적 문제: "~문제가 있다", "~한계가 있다"
     • 성능 한계: "느리다", "낮다", "부족하다", "어렵다"
     • 물리적 한계: "크다", "무겁다", "비싸다"

   - 제외:
     • 배경 설명: "토크 센서는 로봇에 사용된다"
     • 일반론: "일반적으로 정확도가 중요하다"
     • 대조 없는 단순 서술

   - 경계 케이스:
     ✓ "However, 기존 센서는 크고 무겁다" (대조 + 부정적)
     ✓ "기존 방식은 실시간 처리가 어렵다" (기존 + 부정적)
     ✗ "센서는 로봇 관절에 사용된다" (배경 설명)
     ✗ "정확도는 중요한 지표이다" (일반론)

3. method (방법)
   - 정의: 문제 해결을 위해 제안/적용하는 구체적 기술, 알고리즘, 절차
   - 핵심 질문: "어떻게 해결하는가?"
   - 판단 힌트: "~를 적용", "~를 사용", "~기법으로" 등의 표현

   - 구체성 원칙:
     • 가장 구체적인 표현을 추출
     • 더 구체적인 표현이 있으면 상위 개념은 제외
     • 예: "CNN 기반 영상 분류" 있으면 → "딥러닝" 단독 추출 ✗

   - 포함:
     • 고유명사: PointNet++, BERT, KoBERT
     • 대상 + 행위 조합: 표면 코팅, 셀 밸런싱, 차등 신호 처리
     • 단독 고유명사: 더 구체적 표현이 없을 때만 "딥러닝", "CNN" 허용

   - 제외:
     • 단독 행위: 분석, 처리, 학습, 활용
     • 연구 일반론: 실험 방법, 연구 기법, 데이터 분석

   - 경계 케이스:
     ✓ "차등 신호 처리" (대상 + 행위)
     ✓ "PointNet++ 기반 경량화 모델" (고유명사 + 구체적)
     ✓ "딥러닝" (더 구체적 표현 없을 때만)
     ✗ "딥러닝" (CNN 기반 분류가 이미 있으면 제외)
     ✗ "데이터 분석" (단독 행위 + 일반론)

4. objective (목표/효과)
   - 정의: 제안된 방법을 통해 달성한 정량적/정성적 성과
   - 핵심 질문: "무엇을 달성했는가?"
   - 판단 힌트: "달성", "향상", "실현", "개선" 등의 **성과 동사** 필수

   - 필수 조건:
     • 정량적: 반드시 "무엇의" 수치인지 맥락 포함
     • 정성적: "~를 실현했다", "~가 가능해졌다" 형태
     • **성과 동사 필수**: 향상, 달성, 실현, 개선, 감소, 증가 등

   - 포함:
     • 맥락 있는 수치: "사이클 수명 200% 향상", "오차 0.11% 달성"
     • 달성 표현: "실시간 처리 실현", "경량화 달성"

   - 제외:
     • 맥락 없는 수치: "200%", "0.11%", "20시간"
     • 단독 명사: "경량화", "최적화" (달성 표현 없이)
     • 일반론: "성능 향상", "효율 개선" (무엇의?)
     • **측정/조사 대상**: "~를 조사했다", "~를 측정했다", "~를 분석했다"의 대상
       (이들은 target의 속성이므로 별도 추출하지 않음)

   - 경계 케이스:
     ✓ "모멘트 간섭 오차 0.11% 달성" (성과 동사 "달성" 있음)
     ✓ "처리 속도 10배 향상" (성과 동사 "향상" 있음)
     ✗ "0.11%" (무엇의 수치?)
     ✗ "경량화" (달성했는지 목표인지 불분명)
     ✗ "성능 향상" (무엇의 성능?)
     ✗ "토석류 두께 및 속도" (측정 대상 → target "토석류"에 포함)
     ✗ "후방 산란 계수" (분석 대상 → 별도 추출 불필요)
...
1. low_level_keywords (엔티티 검색용)
   - 쿼리에 **직접 언급된** 구체적인 연구 대상, 기술, 방법론
   - 예: 알고리즘명, 소재명, 시스템명, 기술명
   - Local Search에서 개별 엔티티를 직접 찾는 데 사용

2. high_level_keywords (관계 검색용)
   - 쿼리에 **직접 언급된** 엔티티들의 조합/관계 표현
   - "A로 B를 해결", "A를 통한 B 향상", "A 기반 B 개발" 형태
   - Global Search에서 엔티티 간 관계를 찾는 데 사용

---Core Rule: 명시적 추출만---
⚠️ **쿼리에 직접 언급된 키워드만 추출합니다.**

✓ 허용:
- 쿼리에 그대로 나타난 단어/표현
- 쿼리 단어들의 직접 조합 (예: "딥러닝" + "의료영상" → "딥러닝 기반 의료영상")
- 쿼리 표현의 자연스러운 변형 (예: "활용한" → "기반", "위한" → "관련")

✗ 금지:
- 쿼리에 없는 키워드 추론/연상 (예: "의료영상" → "CT", "MRI" 추론 금지)
- 쿼리에 없는 알고리즘명 추론 (예: "머신러닝" → "XGBoost" 추론 금지)
- 쿼리에 없는 관련 분야 추론 (예: "하천" → "수문학" 추론 금지)
- 쿼리에 없는 일반론 생성 (예: "불균형 데이터 처리", "자동 진단")

---Instructions---
- JSON 형식으로 출력
- 모든 키워드는 한국어로 작성
- low_level: 쿼리에서 직접 추출한 명사/명사구 (1-5개)
- high_level: 쿼리 내 키워드들의 직접 조합 (1-3개)
- 쿼리가 단순하면 키워드 수가 적어도 됨
...
################
Output:
{{"low_level_keywords": ["머신러닝", "금융 사기 탐지", "사기 탐지"], "high_level_keywords": ["머신러닝 기반 사기 탐지"]}}
#############################
Example 2:
...
################
Output:
{{"low_level_keywords": ["딥러닝", "의료영상", "의료영상 진단"], "high_level_keywords": ["딥러닝 기반 의료영상 진단"]}}
#############################
Example 3:
...
################
Output:
{{"low_level_keywords": ["배터리", "양극재", "배터리 수명"], "high_level_keywords": ["양극재 배터리 수명 향상"]}}
#############################
Example 4:
...
################
Output:
{{"low_level_keywords": ["기후변화", "하천", "지형", "하천 지형"], "high_level_keywords": ["기후변화 하천 지형"]}}
#############################
Example 5:
...
################
Output:
{{"low_level_keywords": ["IoT", "센서 네트워크", "스케줄링"], "high_level_keywords": ["IoT 센서 네트워크 스케줄링"]}}
#############################
-Real Data-
######################
Query: {query}
